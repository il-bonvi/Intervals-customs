...

{
   // Konfigurationsvariablen
    ...
    // Funktion zur Sicherung der Datenströme
    ...
    const distance = getStreamData("distance"); // Distanzstrom in Metern
    const distanceKm = distance.map(d => d / 1000); // Umrechnung der Entfernung in Kilometer
    const power = getStreamData("fixed_watts"); // Leistungsstrom in Watt
    const heartrate = getStreamData("fixed_heartrate"); // Herzfrequenzstrom in bpm
    const grade = getStreamData("grade_smooth");
    const time = getStreamData("time"); // Zeitstrom (in Sekunden)    
    
    // Anfangswerte des Höhenprofils korrigieren
    ...
    sections.forEach((section, index) => {
        if (currentClimb.startSegment === null && section.grade >= MIN_GRADE) {
            ...
            // Anstieg zurücksetzen
            currentClimb = { startSegment: null, endSegment: null };
        }
    });
      
    // Zusammenfassen von Anstiegen und Lücken als Teil des Anstiegs
    for (let i = 1; i < climbs.length; i++) {
        ...
        if (gapSegments <= MAX_GAP_SEGMENTS) {
            // Lücke und Anstiege zusammenfassen
            prevClimb.endSegment = currentClimb.endSegment;
            climbs.splice(i, 1);
            i--; // Index anpassen, da ein Anstieg entfernt wurde
        }
    }
    
    // Visualisierung der identifizierten Anstiege
    let annotations = [];
    let traces = [];
    let climbNo = 0;
    ...
    climbs.forEach(climb => {
        ...
        // Daten für den gesamten Anstieg berechnen
        ...
        // Anstiege filtern, deren Schwierigkeit unter 5 liegt
        if (difficultyCoefficient < MIN_DIFFICULTY) {
            return; // Diesen Anstieg ignorieren
        }
		...
		const peak5 = climbPowers.reduce((a, b) => a + b, 0) / climbPowers.length;
        ...
        const climbTimeInSeconds = time[endIndex] - time[startIndex]; // Anstiegszeit in Sekunden
        const formattedTime = `${Math.floor(climbTimeInSeconds / 60)}m${climbTimeInSeconds % 60}s`;
        const ascentSpeed = elevationGain / (climbTimeInSeconds / 3600); // Anstiegsgeschwindigkeit in m/h
        const avgHorizontalSpeed = (dist / 1000) / (climbTimeInSeconds / 3600); // Durchschnittliche horizontale Geschwindigkeit in km/h
...
if (vamDiff > 10) {
  ...
  vamArrow = ` ↔️ ${vamDiff.toFixed(0)} m/h`;
        ...
        // Bestimmen der Farbe gemäß der Skala
        let bgColor = "grey";
        ...
        // Trace für den gesamten Anstieg hinzufügen
        traces.push({
...
❤️ ∅ ${avgHR.toFixed(0)} bpm | max. ${maxHR} bpm<br>
🚴‍♂️ ${avgHorizontalSpeed.toFixed(1)} km/h<br> 
⏱️ ${formattedTime}<br>
🚵‍♂️ ${ascentSpeed.toFixed(0)} m/h${vamArrow}<br>
🧮 ${TEORICWKG.toFixed(2)} W/kg | ${TEORICVAM.toFixed(0)} VAM`
        });

        // Dauerhafte Anmerkung: Schwierigkeitskoeffizient (Farbskala)
        annotations.push({
...
text: `QDH ${difficultyCoefficient.toFixed(0)}<br>⚡ ${avgPower.toFixed(0)} <br>❤ ${avgHR.toFixed(0)} <br>⏱️ ${Math.floor(climbTimeInSeconds / 60)}m${climbTimeInSeconds % 60}s`,
    ...
    // Hintergrund-Höhenprofil hinzufügen
    traces.unshift({
        x: distanceKm,
        y: altitude,
