...
function getZoneColor(avgPower, FTP) {
    ...
    const percent = avgPower / FTP;
    if (percent < 0.55) return '#b0e0e6'; // Recovery (light blue)
    if (percent < 0.75) return '#90ee90'; // Endurance (green)
    if (percent < 0.90) return '#ffd700'; // Tempo (yellow)
    if (percent < 1.05) return '#ffa500'; // Sweetspot/Threshold (orange)
    if (percent < 1.20) return '#ff4500'; // VO2max (red-orange)
    if (percent < 1.50) return '#ff0000'; // Anaerobic (red)
    return '#800080'; // Sprint (purple)
}
    ...
    DURATIONS.forEach((WINDOW_SECONDS, dIdx) => {
        ...
        bests.forEach((best, idx) => {
            ...
            traces.push({
                ...
                hoverinfo: 'text',
                visible: true,
                text: `${WINDOW_SECONDS/60}' Effort #${idx + 1}<br>
                ...
                🚴‍♂️ ${avgHorizontalSpeed.toFixed(1)} km/h | 🚵‍♂️ ${ascentSpeed.toFixed(0)} m/h<br>
